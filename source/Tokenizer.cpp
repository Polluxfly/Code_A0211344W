...
map<int, vector<string>> Tokenizer::tokenize(string text, vector<string>& tokens) {
	tokens.clear();
	string token;
	unsigned int rowNum = 0;
	map<int, vector<string>> keyValuePair;

	vector<string> currentRowContent;
	unsigned int i = 0;
	while (i < text.length()) {
		char ch = text.at(i);
		...
		if (isdigit(ch)) { // scan for a numeric sequence
			...
			currentRowContent.push_back(token);
			token.clear();
		}
		...
		{
			keyValuePair[rowNum++] = currentRowContent;
			currentRowContent.clear();
			i++;
		}
		else {
			i++;
